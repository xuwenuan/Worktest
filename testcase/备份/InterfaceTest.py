...
from PyQt5.QtWidgets import QComboBox, QAction, QMenu
...
class InterfaceTest(QMainWindow, UI2.Ui_MainWindow):
    config = CanLinConfig()
    ...
    def __init__(self, parent = None):
        ...
        # self.pushButton_19.clicked.connect(self.clear_table)
        # self.pushButton_20.clicked.connect(self.export_to_excel)
        # self.pushButton_21.clicked.connect(self.readMesseage)
    ...
    #             current_col = sheet.ncols + 1
    #             self.table.setRowCount(current_row)
    #             self.table.setColumnCount(current_col)
    #             # 填充表格数据
    #             for row in range(1, sheet.nrows):  # 从第 2 行开始（索引 1）
    #                 for col in range(sheet.ncols):
    #                     try:
    #                         # 将单元格值转换为字符串并设置到表格中
    #                         item = QTableWidgetItem(str(sheet.cell_value(row, col)))
    #                         self.table.setItem(row - 1, col, item)
    #                     except Exception as e:
    #                         logging.error(f"填充表格数据时发生错误 (行: {row}, 列: {col}): {traceback.format_exc()}")
    #                         QMessageBox.critical(None, "错误",f"填充表格数据时发生错误 (行: {row}, 列: {col}): {str(e)}")
    #             # 添加删除按钮到每一行
    #             for row in range(current_row):
    #                 try:
    #                     deleteButton = QPushButton("删除")
    #                     deleteButton.clicked.connect(self.delete_clicked)
    #                     self.table.setCellWidget(row, current_col - 1, deleteButton)  # 假设最后一列为操作列
    #                 except Exception as e:
    #                     logging.error(f"添加删除按钮时发生错误 (行: {row}): {traceback.format_exc()}")
    #                     QMessageBox.critical(None, "错误", f"添加删除按钮时发生错误 (行: {row}): {str(e)}")
    #     except:
    #         logging.error(traceback.format_exc() + "\n")
    #         QMessageBox.critical(None, "错误", f"添加行时发生错误: {traceback.format_exc()}")


    def addrow(self):
        try:
            current_row_count = self.tableWidget_2.rowCount()
            print(f"当前行数: {current_row_count}")
            self.tableWidget_2.insertRow(current_row_count)  # 先插入行
            deleteButton = QPushButton("删除")
            deleteButton.clicked.connect(self.delete_clicked)
            self.tableWidget_2.setCellWidget(current_row_count, 10, deleteButton)  # 再设置按钮
        except Exception as e:
            logging.error(traceback.format_exc())
            QMessageBox.critical(self, "错误", f"添加行时发生错误: {str(e)}")
    ...
    def genautotable(self):
        ...
        for row in range(self.tableWidget.rowCount()):
            ...
            if item[5] == 'IN':  # 信号方向为 IN
                ...
                num = '01' if item[0] in ('UInt8', 'Boolean') else '81' if item[0] == 'SInt8' else '02' if item[0] == 'UInt16' else '82' if item[0] == 'SInt16' else '00'
                ...
                num = '11' if item[0] in ('UInt8', 'Boolean') else '91' if item[0] == 'SInt8' else '12' if item[0] == 'UInt16' else '92' if item[0] == 'SInt16' else '00'
        ...
        if not has_dataList :
            ...
            file_path1, _ = QFileDialog.getSaveFileName(self, '导出id路由自动测试', '', 'Excel File(*.xlsx)')
