...
class MainWindow(QMainWindow,Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        # 按钮配置表
            button_config = [
                {"name": "pushButton_2", "signal": "clicked", "callback": lambda: self.addrow(0)},
                {"name": "action_xlsx", "signal": "triggered", "callback": self.open_file},
                {"name": "pushButton", "signal": "clicked", "callback": self.genmessage},
                {"name": "pushButton_4", "signal": "clicked", "callback": self.saveToexcel},
                {"name": "pushButton_5", "signal": "clicked", "callback": lambda: self.addnote1(0)},
                {"name": "pushButton_3", "signal": "clicked", "callback": self.clear_row},
                {"name": "action_xlsx1", "signal": "triggered", "callback": self.excelTotable},
                {"name": "action_xlsx4", "signal": "triggered", "callback": self.genAutoExcel},
            ]

        # 动态绑定信号槽
            for config in button_config:
                button = getattr(self, config["name"])  # 获取按钮对象
                signal = getattr(button, config["signal"])  # 获取信号
                signal.connect(config["callback"])  # 绑定回调函数
    ...
    def addwidget(self):
        ...
        layout2=RoutingTest()
        layout2.pushButton.clicked.connect(lambda: layout2.readMesseage(self.file.name))
    ...
    def contextMenuEvent(self,event):
        try:
            ...
            copy_action = menu.QAction("复制 (Ctrl+C)", self)
            copy_action.triggered.connect(self.copy_selected_cells)
            paste_action = menu.QAction("粘贴 (Ctrl+V)", self)
            paste_action.triggered.connect(self.paste_cells)

            context_menu.addAction(copy_action)
            context_menu.addAction(paste_action)
            context_menu.exec_(self.blank_table.viewport().mapToGlobal(event))
    ...
    def copy_to_blank_table(self, data_frame):
        """将导入的表格内容复制到空白表格中"""
        self.blank_table.setRowCount(data_frame.shape[0])
        self.blank_table.setColumnCount(data_frame.shape[1])
        self.blank_table.setHorizontalHeaderLabels(data_frame.columns)

        for row in range(data_frame.shape[0]):
            for col in range(data_frame.shape[1]):
                item = QTableWidgetItem(str(data_frame.iat[row, col]))
                self.blank_table.setItem(row, col, item)

    def copy_selected_cells(self):
        """复制选中的单元格内容到剪贴板"""
        selected_ranges = self.blank_table.selectedRanges()
        ...
        for row in range(top_row, bottom_row + 1):
            ...
            for col in range(left_col, right_col + 1):
                item = self.blank_table.item(row, col)
    ...
    def paste_cells(self):
        ...
        for i, row_data in enumerate(data):
            for j, cell_value in enumerate(row_data):
                ...
                if target_row < self.blank_table.rowCount() and target_col < self.blank_table.columnCount():
                    item = QTableWidgetItem(cell_value)
                    self.blank_table.setItem(target_row, target_col, item)
