...
class SearchThread(QThread):
    progress = pyqtSignal(int)              # 搜索进度（0~100）
    result = pyqtSignal(bool)               # 是否找到结果
    highlight_all = pyqtSignal(list)        # 所有匹配项 [(row, col), ...]
    error = pyqtSignal(str)                 # 错误信息
    finished_signal = pyqtSignal()          # 搜索完成

    def __init__(self, table_widget, keyword_lower, columns):
        super().__init__()
        self.table_widget = table_widget
        self.keyword_lower = keyword_lower
        ...
        try:
            ...
            for row in range(row_count):
                ...
                for col in self.columns:
                    ...
                    if item and self.keyword_lower in item.text().lower():
                        self.matches.append((row, col))
                        found = True

                # 每20行更新一次进度
                if row_count > 50 and row % 20 == 0:
                    self.progress.emit(int((row + 1) * 100 / row_count))

            self.result.emit(found)
            self.highlight_all.emit(self.matches)  # 一次性发出所有匹配项
        except Exception as e:
...
class UiMain(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        #CAN配置界面
        self.pushButton_8.clicked.connect(self.refresh_connected_devices)
        self.comboBox_2.currentIndexChanged.connect(self.on_combobox_changed)

        self.pushButton_16.clicked.connect(self.test_sendmessage)
        # 重定向标准输出
        ...
        self.pushButton_21.clicked.connect(lambda :self.Interface.read_Messeage(self.filename))
    ...
    def get_table_data(self):
        ...
        # 如果是 CAN 配置页面（page_5），直接返回空数据，避免弹窗提醒
        if current_index == 0:
            return []
    ...
    def search_in_table(self, keyword, columns=None):
        if len(keyword.strip()) < 2:
            QMessageBox.information(self, "提示", "关键词过短，可能导致性能问题")
            return

        current_index = self.stackedWidget.currentIndex()
        config_map = {
            1: (self.tableWidget_5, None),
            2: (self.tableWidget_2, None),
            # 3: (self.tableWidget_3, [1, 2, 8, 9]),
            # 4: (self.tableWidget_4, [3, 15]),
            3: (self.tableWidget_3, None),
            4: (self.tableWidget_4, None),
        }

        if current_index not in config_map:
            QMessageBox.warning(self, "警告", "当前页面没有对应的表格控件")
            return

        table_widget, default_columns = config_map[current_index]

        keyword_lower = keyword.lower()
        if columns is None:
            columns = range(table_widget.columnCount()) if default_columns is None else default_columns
        ...
        # 启动新线程
        self.search_thread = SearchThread(table_widget, keyword_lower, columns)
        self.search_thread.highlight_all.connect(lambda matches: self.highlight_all_cells(table_widget, matches))
        self.search_thread.progress.connect(self.update_progress)
        self.search_thread.result.connect(self.show_search_result)
        self.search_thread.error.connect(self.handle_search_error)
        self.search_thread.finished_signal.connect(self.cleanup_search)

        self.progress_dialog = QProgressDialog("搜索中...", "取消", 0, 100, self)
        self.progress_dialog.setWindowModality(Qt.WindowModal)
        self.progress_dialog.setWindowTitle("搜索进度")
        self.progress_dialog.canceled.connect(self.on_cancel_search)
        self.progress_dialog.show()

        self.search_thread.start()

    def show_search_result(self, found):
        if not found:
            QMessageBox.information(None, "搜索结果", f"未找到关键字")
        else:
            QMessageBox.information(None, "搜索结果", f"已高亮显示关键字")

    def highlight_all_cells(self, table_widget, matches):
        for row, col in matches:
            item = table_widget.item(row, col)
            if item:
                item.setBackground(QBrush(QColor(255, 255, 0)))

    def highlight_cell(self, table_widget, row, col):
        item = table_widget.item(row, col)
        if item:
            item.setBackground(QBrush(QColor(255, 255, 0)))
    ...
    def handle_search_error(self, error_msg):
        logging.error(f"搜索异常: {error_msg}")
    ...
    def insert_selected_cells(self):
        try:
            ...
            if  current_index == 2:
                table_widget = self.tableWidget_2
                selected_ranges = self.tableWidget_2.selectedRanges()
                if not selected_ranges:
                    QMessageBox.warning(None, "警告", "未选中任何单元格")
                    return
                selected_row = selected_ranges[0].bottomRow()
                insert_row = selected_row + 1
                self.Interface.add_row_3(insert_row)
                return
            elif current_index == 3:
                table_widget = self.tableWidget_3
            elif current_index == 4:
    ...
    def cleanup_resources(self):
        ...
        # 示例：断开信号与槽连接
        try:
            self.tableWidget_2.cellChanged.disconnect(self.record_operation)
            self.tableWidget_3.cellChanged.disconnect(self.record_operation)
            self.tableWidget_4.cellChanged.disconnect(self.record_operation)
            self.tableWidget_5.cellChanged.disconnect(self.record_operation)
        except Exception as e:
        ...
        try:
            ...
            for i in range(1, sheet.nrows):
                ...
                if values[8] != '' and values[0] != '' and values[1] == '':
                    length.append(int(values[13]))
                    Canlist.append(values[8])
            ...
            self.Canlength = {signal: length[i] for i, signal in enumerate(Canlist)}

            # print(self.Canlength)
            ...
            for i in range(1, sheet5.nrows):
                ...
                if values[8] != '' and values[0] != '' and values[1] == '':
                    try:
                        ...
                        length.append(0)  # 如果无法转换为 int，设置为 0 或其他默认值

                    LINlist.append(values[8])
            ...
            # 构建 Linlength 字典
            self.Linlength = {signal: length[i] for i, signal in enumerate(LINlist) if i < len(length)}

            # print(self.Linlength)
            ...
            # 调用 copy_datalist 方法时使用类属性
            self.Interface.copy_datalist(datalist, linlist, self.Canlength, self.Linlength)
    ...
    def start_auto_save(self):
        self.auto_save_thread = threading.Thread(target=self.save_file_periodically, daemon=True)
    ...
    def save_file_periodically(self):
        try:
            while True:
                ...
                if current_index == 1:
                    self.GeneralTest.excel_to_excel(filepath='./功能点检测试用例（自动保存）.xlsx')
                    print(f"[{timestamp}] 自动保存：功能点检测试用例")
                elif current_index == 2:
                    self.Interface.excel_to_excel(filepath='./接口测试用例（自动保存）.xlsx')
                    print(f"[{timestamp}] 自动保存：接口测试用例")
                elif current_index == 3:
                    self.RoutingTest.excel_to_excel(filepath='./ID路由测试用例（自动保存）.xlsx')
                    print(f"[{timestamp}] 自动保存：ID路由测试用例")
                elif current_index == 4:
                    self.SignalRoutingTest.excel_to_excel(filepath='./信号路由测试用例（自动保存）.xlsx')
                    print(f"[{timestamp}] 自动保存：信号路由测试用例")
                ...
                if msg_fd.FProperties & 1 == 0:  # 仅处理 RX 报文
                    try:
                        ...
                        if expected_data == received_data:
                            ...
                            print(
                                f"Check 失败: ID 0x{msg_fd.FIdentifier:03X}, 期望 {expected_data}, 实际 {received_data}")
                    except Exception as e:
