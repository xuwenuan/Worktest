    ...
    def add_rows(self):
        try:
            ...
            for _ in range(row_count):
                ...
                self.table.setCellWidget(current_row_count, 22, delete_button)
                current_row_count += 1
    ...
    def excel_to_excel(self, filepath):
        try:
            if self.table.rowCount() == 0:
                logging.warning(f"表格为空，未保存: {filepath}")
                return False
            ...
            headers = ['Name', 'Hyt', 'CANFD', 'ID', 'dlc', 'cycle', 'segment', 'startbit', 'length',
                       'Reserved1', 'Reserved2', 'Reserved3', 'Name_2', 'Hyt_2', 'CANFD_2', 'ID_2',
                       'dlc_2', 'cycle_2', 'segment_2', 'startbit_2', 'length_2', 'Reserved4', '操作']
            for col_num, header in enumerate(headers, 1):
                sheet.cell(row=1, column=col_num, value=header)

            max_col = len(headers)
            for row_num in range(self.table.rowCount()):
                for col_num in range(min(self.table.columnCount(), max_col)):
                    item = self.table.item(row_num, col_num)
