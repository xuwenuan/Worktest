    ...
    def run(self):
        try:
            ...
            for i in range(1, sheet.nrows):
                ...
                # 更新进度
                progress = int((i / (sheet.nrows - 1)) * 100)
...
class RoutingTest:
    ...
    def excel_toTable(self):
        try:
            ...
            # 显示进度对话框
            self.progress_dialog = QProgressDialog("文件导入中，请稍候...", "取消", 0, 100, None)
        ...
        if data:
            ...
            for i, row_data in enumerate(data):
                ...
                for j, value in enumerate(row_data):
                    ...
                    self.table.setItem(row_count + i, j, QTableWidgetItem(text))

            QMessageBox.information(None, "导入成功", f"文件导入成功，共 {len(data)} 行。")
        else:
            QMessageBox.critical(None, "导入失败", "文件导入失败，详细信息请查看日志。")
    ...
    def export_to_excel(self):
        try:
            ...
            for row_num in range(self.table.rowCount()):
                for col_num in range(self.table.columnIntCount()):
            ...
            if not has_content:
                QMessageBox.warning(None, "警告", "表格中没有可导出的有效数据，导出失败！")
                return
    ...
    def excel_to_excel(self, filepath):
        try:
            if self.table.rowCount() == 0:
                logging.warning(f"未保存，表格为空: {filepath}")
                return False
            ...
            headers = ['源网段', '源节点', '源ID', '源端是否CANFD', '源DLC', '源周期', '源字段',
                       '目标网段', '目标节点', '目标ID', '目标端是否CANFD', '目标DLC',
                       '目标周期', '目标字段', '报文路由类型', 'MsgRoutType', '测试使能', '可选项1', '操作']
            for col_num, header in enumerate(headers, 1):
                sheet.cell(row=1, column=col_num, value=header)
