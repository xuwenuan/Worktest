...
#原程序中tsapp_set_mapping_verbose（*）中引入的参数为不带_的版本，其对应的类型为INT32，无法访问到其对应的属性
#因此，直接访问其类型，并没有实例化对象  直接访问属性  所以通过from进行函数导入
#具体效果如何还没有经过具体的硬件连接进行测试
from TSMasterAPI.TSEnum import (_TLIBApplicationChannelType, _TLIBBusToolDeviceType,
                                _TLIB_TS_Device_Sub_Type, _TLIBCANFDControllerType,
                                _TLIBCANFDControllerMode, _CHANNEL_INDEX)
...
def connect():
    ...
    #用于检测传入的类型和是否存在其属性
    # print('类型是',type(TLIBApplicationChannelType.value))
    # #hasattr检测是否存在其属性
    # if hasattr(TLIBApplicationChannelType, 'APP_CAN'):
    #     print('TLIBApplicationChannelType.APP_CAN is exist')
    # else:
    #     print('TLIBApplicationChannelType.APP_CAN is not exist')

    # 硬件通道映射至软件通道
    # tosun其他硬件只需修改第6个参数，找到对应型号即可
    if 0 == tsapp_set_mapping_verbose(AppName, _TLIBApplicationChannelType.APP_CAN, _CHANNEL_INDEX.CHN1,
                                      "TC1016".encode("UTF8"), _TLIBBusToolDeviceType.TS_USB_DEVICE,
                                      _TLIB_TS_Device_Sub_Type.TC1016,0, 0, True):
        print("1通道映射成功")
    else:
        print("1通道映射失败")

    # 设置canfd波特率
    if 0 == tsapp_configure_baudrate_canfd(_CHANNEL_INDEX.CHN1, 500.0, 2000.0,
...
def receive_can_message():
    ...
    r = tsfifo_receive_can_msgs(listcanmsg, cansize, 0, 1)#READ_TX_RX_DEF.TX_RX_MESSAGES
    r = tsfifo_receive_canfd_msgs(listcanfdmsg, canfdsize, 0, 1)#READ_TX_RX_DEF.TX_RX_MESSAGES
    print("接收返回值=", r)

    #遍历实际接收的报文数量（使用.value获取整数值）
    # 源程序类型错误，由cansize修改为cansize.value
    for i in range(cansize.value):
        print("fifo接收canID=", listcanmsg[i].FIdentifier)

    # 原程序类型错误，由canfdsize修改为canfdsize.value
...
#上面程序报type_eeor其中id1类型不一致，所以报错
def load_dbc():
    global id1
    root = tk.Tk()
    root.withdraw()
    filepath = filedialog.askopenfilename()
    print(f"Selected filepath: {filepath}, type: {type(filepath)}")  # 添加调试信息

    if filepath and filepath.endswith(".dbc"):
        # 确保 id1 是一个指向 u32 类型的指针
        id1 = c_uint32(0)
        id1_ptr = pointer(id1)

        r = tsdb_load_can_db(filepath.encode(), "0,1".encode(), id1_ptr)  # 确保传递的是字节串
        if r == 0:
            print("id1 =", id1.value)  # 打印实际的值
            print(f"{filepath[filepath.rindex('/') + 1:]} 文件加载成功")
            return filepath
        else:
            print(f"加载失败，返回码: {r}")
            return 0
...
def creat_uds_module():
    global udsHandle
    r = tsdiag_can_create(udsHandle, _CHANNEL_INDEX.CHN1, 0, 8, 0X1, True, 0X2, True, 0X3, True)
    if r == 0:
