...
from PyQt5.QtGui import QBrush, QColor, QCursor, QPalette
from PyQt5.QtWidgets import QAbstractItemView, QApplication, QMainWindow, QHeaderView, QFileDialog, QTableWidgetItem, \
    QPushButton, \
    QCompleter, QMessageBox, QLineEdit, QMenu, QComboBox, QTableWidget
...
class MainWindow(QMainWindow,Ui_MainWindow):
    ...
    # def addSetting(self):
    #     self.win = NewWindow4()
    #     self.win.Level1=self.chapterLevel1
    #     self.win.Level2 = list(set(self.chapterLevel2))
    #     self.win.Level3 = list(set(self.chapterLevel3))
    #     self.win.Level4 = list(set(self.chapterLevel4))
    #     self.win.setwin()
    #     self.win.pushButton.clicked.connect(self.insertChapter)
    #     self.win.show()
    ...
    #     try:
    #         path = QFileDialog.getOpenFileName(self, "选择文件", '/', "docx Files(*.docx)")
    #         doc = Document(path[0])
    #         level1 = []
    #         level2 = []
    #         level3 = []
    ...
    #             if para.style.name.startswith('Heading') and para.style.name.replace('Heading', '').strip() == '1':
    #                 level = para.style.name.replace('Heading', '').strip()  # 获取标题级别
    #                 title = para.text.strip().replace('              ', '').replace('\t','')  # 获取标题内容
    #                 level1.append(f"Level {level}: {title}")
    #                 self.chapterLevel1.append(title)
    #             elif para.style.name.startswith('Heading') and para.style.name.replace('Heading', '').strip() == '2':
    #                 level = para.style.name.replace('Heading', '').strip()  # 获取标题级别
    #                 title = para.text.strip().replace('              ', '').replace('\t','')  # 获取标题内容
    #                 data = level1[-1] + ';Level ' + level + ": " + title
    #                 # data = '1.'+title
    #                 level2.append(data)
    #                 self.chapterLevel2.append(title)
    #             elif para.style.name.startswith('Heading') and para.style.name.replace('Heading', '').strip() == '3':
    #                 level = para.style.name.replace('Heading', '').strip()  # 获取标题级别
    #                 title = para.text.strip().replace('              ', '').replace('\t','')  # 获取标题内容
    #                 data = level2[-1] + ';Level ' + level + ": " + title
    #                 level3.append(data)
    #                 self.chapterLevel3.append(title)
    #             elif para.style.name.startswith('Heading') and para.style.name.replace('Heading', '').strip() == '4':
    #                 level = para.style.name.replace('Heading', '').strip()  # 获取标题级别
    #                 title = para.text.strip().replace('              ', '').replace('\t','')  # 获取标题内容
    #                 data = level3[-1] + ';Level ' + level + ": " + title
    #                 level4.append(data)
    #                 self.chapterLevel4.append(title)
    #             # else:
    #             #     print(para.style.name.replace('Heading', '').strip() +':'+para.text)
    #         dataList = level1 + level2 + level3 + level4
    #         self.chapterList = dataList
    #         msg_box = QMessageBox(QMessageBox.Information, "标题", "功能规范导入成功!")
    #         msg_box.exec_()
    #     except:
    #         logging.error(traceback.format_exc() + "\n")
    ...
    def getELFAnalysis(self):
        ...
        if path[0]:
            # elfAnalysis = ELFAnalysis()
            self.elfAnalysis.loadPath(path[0])
    ...
    def contextMenuEvent(self,event):
        try:
            ...
            # title_row_action = menu.addAction('插入章节')
            ...
            # title_row_action.triggered.connect(lambda: self.addchapter1(row + 1))
            ...
            for i in range(row):
                ...
                if self.tableWidget.cellWidget(i,0) and self.tableWidget.cellWidget(i,1).currentText()!='软件接口测试' and self.tableWidget.cellWidget(i,0).currentText()!='配置':
                    ...
                    # 参数在Cansignalname
                        ...
                        data4=' 00 00'
                    if self.tableWidget.cellWidget(i,3) and self.tableWidget.cellWidget(i,3).currentText()=='保持':
                        data5 = ' FF FF'
                    elif self.tableWidget.cellWidget(i,5) and self.tableWidget.cellWidget(i,5).text()!='':
                        if self.tableWidget.cellWidget(i,5).text()=='FFFF':
                            data5 = ' FF FF'
                        else:
                    ...
                    if self.tableWidget.cellWidget(i,0).currentText()!='配置' and (self.tableWidget.cellWidget(i,1).currentText() in self.CAN or self.tableWidget.cellWidget(i,1).currentText() in self.LIN):
                        ...
                        if self.tableWidget.cellWidget(i,1).currentText() in self.CAN+self.LIN:
                            ...
                            data7 = self.config.getMessage(self.startBit,self.length,num,8)
                        if self.tableWidget.cellWidget(i,1).currentText() in self.CAN:
